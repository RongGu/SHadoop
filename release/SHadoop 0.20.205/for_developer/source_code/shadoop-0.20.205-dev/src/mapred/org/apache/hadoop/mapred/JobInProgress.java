...
public class JobInProgress {
  ...
  throws IOException, KillInterruptedException, UnknownHostException {
   ...
   // move Task.runJobSetupTask() here
   setupJob();
   this.changeStateTo(JobStatus.RUNNING);
   this.setup[0].setCompleted();
   //TODO modified by yang xiaoliang
   // contact the tasktrackers at once
   informTaskTrackersInstantly(splits);
  }

  //
  
 //TODO: modified by yang xiaoliang
 // moved the function form Task here
  private void setupJob() throws IOException{
	  LOG.info("setupJob() called");
      String outputPath = conf.get("mapred.output.dir");
      Path tmpDir = new Path(outputPath, FileOutputCommitter.TEMP_DIR_NAME);
      FileSystem fileSys = tmpDir.getFileSystem(this.conf);
   ...
   */
  //private
  void informTaskTrackersInstantly(TaskSplitMetaInfo[] splits){
	  Random random = new Random(System.currentTimeMillis());
	  for(TaskSplitMetaInfo split: splits){
		  String host = split.getLocations()[
		          (int)(random.nextDouble()*split.getLocations().length)];
		  
		  Thread informTaskTrackerThread = 
				  new Thread(new InformTaskTracker(host));
		  informTaskTrackerThread.start();
		  LOG.info("inform tracker: "+host);
	  }
  }
  private class InformTaskTracker implements Runnable{
	  
	  String taskTracker = null;
	  
	  public InformTaskTracker(String taskTracker){
	  ...
	  public void run(){
		  if(this.taskTracker != null){
			  InstantContactProtocol instantTaskTracker=null;
			try {
				instantTaskTracker = (InstantContactProtocol)RPC.getProxy(
				InstantContactProtocol.class, InstantContactProtocol.versionID, 
				new InetSocketAddress(taskTracker,9010), 
				JobInProgress.this.conf);
				org.apache.hadoop.mapred.TaskTracker.State state = 
						instantTaskTracker.contactJobTracker();
  ...
  public Task obtainJobCleanupTask(TaskTrackerStatus tts, 
    ...
    //TODO modified by yang xiaoliang
    LOG.info("canLaunchJobCleanupTask="+canLaunchJobCleanupTask());
    synchronized(this) {
  ...
  private synchronized boolean canLaunchJobCleanupTask() {
    ...
    // zero.
    //TODO modified by yang xiaoliang
    LOG.info("isSetupFinished()="+isSetupFinished());
...
//  //TODO: modyfied by yang xiaoliang
//    // 2012-05-16
    if(canLaunchJobCleanupTask())
    {
      try {
  ...
  /**
   * added by yang xiaoliang
   * move Task.runJobCleanupTask() here
   * @throws IOException 
   */
  private void cleanupJob() throws IOException{
    ...
    if(status.getRunState() == JobStatus.FAILED
      ...
      // equals to FileOutPutCommitter.abort()
      LOG.info("Aborting job with runstate : " + status.getRunState());
      if (outputPath != null) {
        Path tmpDir = new Path(outputPath, FileOutputCommitter.TEMP_DIR_NAME);
        FileSystem fileSys = tmpDir.getFileSystem(this.conf);
      ...
      if (outputPath != null) {
        ...
        if (conf.getBoolean("mapreduce.fileoutputcommitter.marksuccessfuljobs", 
            true)){
      ...
      throw new IOException("Invalid state of the job for cleanup. State found "
          +this.status.getRunState()+" expecting "
          + org.apache.hadoop.mapreduce.JobStatus.State.SUCCEEDED + ", " 
          + org.apache.hadoop.mapreduce.JobStatus.State.FAILED + " or "
          + org.apache.hadoop.mapreduce.JobStatus.State.KILLED);
    }
    
   // delete the staging area for the job
    if(!(conf.getKeepTaskFilesPattern() != null || conf
